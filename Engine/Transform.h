...
{
	XMVECTOR pv = XMLoadFloat3(&p);//アドレスを渡してるから&がついている
	pv += v;
	XMStoreFloat3(&p, pv);
	return p;
}

inline XMFLOAT3 operator+(const XMFLOAT3& p1, const XMVECTOR& v2)
{
	XMVECTOR pv = XMLoadFloat3(&p1);//アドレスを渡してるから&がついている
	pv += v2;
	XMFLOAT3 out;
